...
def test_subsequent_phase_prompts_read_the_frozen_model():
    ...
    decision_guidance = SCRIPT.split('$scientificModelDecisionGuidance = "', 1)[1].split(
        '"', 1
    )[0]
    assert "research/scientific_model.md" in decision_guidance
    assert "training" in decision_guidance and "measurement" in decision_guidance
    for prompt in ("analysisPrompt", "evaluationPrompt", "decisionPrompt", "researchPrompt"):
        ...
        assert ("$scientificModelDecisionGuidance" in corpus) is (
