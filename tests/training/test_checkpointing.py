...
def test_candidate_checkpoint_records_rolling_metrics_at_save_time(
    tmp_path, monkeypatch
):
    # This test covers rolling metrics; executable artifact export has independent
    # integration tests with real SB3 weights and normalization, not empty files.
    monkeypatch.setattr(
        "robot_learning.training.candidate_checkpoint_callback.save_checkpoint",
        lambda model, env, directory: None,
    )
    ...
    class Model:
        def __init__(self):
            self.ep_info_buffer = [{"r": 0.0}, {"r": 4.0}]
            self.ep_success_buffer = [0.0, 1.0]
    ...
    assert (
        tmp_path / "candidate_pool" / "checkpoint-1" / "training_metrics.json"
    ).read_text(
        encoding="utf-8"
    ) == '{\n  "success_rate": 0.5,\n  "ep_rew_mean": 2.0\n}\n'
