...
def reach_observation(data) -> np.ndarray:
    ...
    inverse_kinematics_encoding = np.asarray(
        [
            component
            for error in inverse_kinematics_errors
            for component in (np.sin(wrap_to_pi(error)), np.cos(wrap_to_pi(error)))
        ]
    )
