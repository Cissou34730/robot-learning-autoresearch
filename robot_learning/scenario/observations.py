...
def shoulder_for_elbow(target_x: float, target_y: float, elbow: float) -> float:
    return float(
...
def joint_limit_margin(angle: float) -> float:
    wrapped = wrap_to_pi(angle)
    return min(wrapped + JOINT_LIMIT_RADIANS, JOINT_LIMIT_RADIANS - wrapped)
...
def reach_observation(data) -> np.ndarray:
    ...
    shoulder_open = shoulder_for_elbow(target_x, target_y, elbow_open)
    elbow_folded = -elbow_open
    shoulder_folded = shoulder_for_elbow(target_x, target_y, elbow_folded)
    ...
    return np.concatenate(
        [
            ...
            [
                min(joint_limit_margin(shoulder_open), joint_limit_margin(elbow_open)),
                min(
                    joint_limit_margin(shoulder_folded),
                    joint_limit_margin(elbow_folded),
                ),
            ],
        ]
    ).astype(np.float32)
