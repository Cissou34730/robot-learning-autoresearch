...
TRAINING_FOCUSED_ANGLE_RANGE = (-5.0 * np.pi / 6.0, -2.0 * np.pi / 3.0)
TRAINING_FOCUSED_ANGLE_FRACTION = 0.5
...
class TwoJointArmReachEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        focused_angle_fraction: float = 0.0,
        policy_runtime=None,
    ) -> None:
        super().__init__()
        if not 0.0 <= focused_angle_fraction <= 1.0:
            raise ValueError("focused_angle_fraction must be between 0 and 1")
        self.max_episode_steps = max_episode_steps
        self.frame_skip = frame_skip
        self.target_radius_range = target_radius_range
        self.focused_angle_fraction = focused_angle_fraction
    ...
    def _sample_target_position(self) -> None:
        if self.np_random.random() < self.focused_angle_fraction:
...
def make_training_env() -> gym.Env:
    ...
    return TwoJointArmReachEnv(
        ...
        focused_angle_fraction=TRAINING_FOCUSED_ANGLE_FRACTION,
    )
