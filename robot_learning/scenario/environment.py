...
TRAINING_OUTER_TARGET_PROBABILITY = 0.5
TRAINING_OUTER_TARGET_RADIUS_RANGE = (0.14, 0.20)
...
class TwoJointArmReachEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        outer_target_probability: float = 0.0,
        outer_target_radius_range: tuple[float, float] = TRAINING_OUTER_TARGET_RADIUS_RANGE,
        policy_runtime=None,
    ) -> None:
        super().__init__()
        ...
        self.outer_target_probability = float(outer_target_probability)
        self.outer_target_radius_range = outer_target_radius_range
        if not 0.0 <= self.outer_target_probability <= 1.0:
            raise ValueError("outer_target_probability must be between 0 and 1")
    ...
    def _sample_target_position(self) -> None:
        ...
        radius_range = self.target_radius_range
        if self.np_random.random() < self.outer_target_probability:
            radius_range = self.outer_target_radius_range
        radius = float(self.np_random.uniform(radius_range[0], radius_range[1]))
...
def make_training_env() -> gym.Env:
    ...
    return TwoJointArmReachEnv(
        ...
        outer_target_probability=TRAINING_OUTER_TARGET_PROBABILITY,
    )
