...
TRAINING_FOCUSED_ANGLE_RANGE = (-np.pi, 0.0)
...
class TwoJointArmReachEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        focused_angle_range: tuple[float, float] | None = None,
        focused_angle_probability: float = 0.0,
        policy_runtime=None,
    ) -> None:
        super().__init__()
        if not 0.0 <= focused_angle_probability <= 1.0:
            raise ValueError("focused_angle_probability must be between 0 and 1")
        if focused_angle_range is not None and (
            focused_angle_range[0] >= focused_angle_range[1]
            or focused_angle_range[0] < -np.pi
            or focused_angle_range[1] > np.pi
        ):
            raise ValueError("focused_angle_range must be ordered within [-pi, pi]")
        self.max_episode_steps = max_episode_steps
        self.frame_skip = frame_skip
        self.target_radius_range = target_radius_range
        self.focused_angle_range = focused_angle_range
        self.focused_angle_probability = focused_angle_probability
    ...
    def _sample_target_position(self) -> None:
        use_focused_angle = (
            self.focused_angle_range is not None
            and self.np_random.random() < self.focused_angle_probability
        )
        angle_range = (
            self.focused_angle_range if use_focused_angle else (-np.pi, np.pi)
        )
...
def make_training_env() -> gym.Env:
    ...
    return TwoJointArmReachEnv(
        ...
        focused_angle_range=TRAINING_FOCUSED_ANGLE_RANGE,
        focused_angle_probability=TRAINING_FOCUSED_ANGLE_PROBABILITY,
    )
