...
TRAINING_HARD_ANGLE_RANGE = (
    float(np.deg2rad(-165.0)),
    float(np.deg2rad(-105.0)),
)
...
class TwoJointArmReachEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        hard_angle_range: tuple[float, float] | None = None,
        hard_angle_probability: float = 0.0,
        policy_runtime=None,
    ) -> None:
        super().__init__()
        if not 0.0 <= hard_angle_probability <= 1.0:
            raise ValueError("hard_angle_probability must be between 0 and 1")
        if hard_angle_range is not None and hard_angle_range[0] >= hard_angle_range[1]:
            raise ValueError("hard_angle_range must be increasing")
        self.max_episode_steps = max_episode_steps
        self.frame_skip = frame_skip
        self.target_radius_range = target_radius_range
        self.hard_angle_range = hard_angle_range
        self.hard_angle_probability = hard_angle_probability
    ...
    def _sample_target_position(self) -> None:
        if (
            self.hard_angle_range is not None
            and self.np_random.random() < self.hard_angle_probability
        ):
            angle = float(self.np_random.uniform(*self.hard_angle_range))
        else:
...
def make_training_env() -> gym.Env:
    ...
    return TwoJointArmReachEnv(
        ...
        hard_angle_probability=TRAINING_HARD_ANGLE_PROBABILITY,
    )
