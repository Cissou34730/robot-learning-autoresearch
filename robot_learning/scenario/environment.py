...
TRAINING_FOCUS_ANGLE_RANGE = (np.deg2rad(-165.0), np.deg2rad(-90.0))
...
class TwoJointArmReachEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        *,
        ...
        focus_angle_probability: float = 0.0,
        policy_runtime=None,
    ) -> None:
        super().__init__()
        ...
        self.focus_angle_probability = focus_angle_probability
    ...
    def _sample_target_position(self) -> None:
        if self.np_random.random() < self.focus_angle_probability:
            angle = float(self.np_random.uniform(*TRAINING_FOCUS_ANGLE_RANGE))
        else:
...
def make_training_env() -> gym.Env:
    ...
    return TwoJointArmReachEnv(
        ...
        focus_angle_probability=TRAINING_FOCUS_ANGLE_PROBABILITY,
    )
