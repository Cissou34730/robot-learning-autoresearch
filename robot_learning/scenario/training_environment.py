...
FOCUSED_TARGET_ANGLE_RANGE = tuple(np.deg2rad((-160.0, -110.0)))
FOCUSED_TARGET_PROBABILITY = 0.5


class ReachTrainingEnv(TwoJointArmReachEnv):
    def _sample_target_position(self) -> None:
        if self.np_random.random() < FOCUSED_TARGET_PROBABILITY:
            angle = float(
                self.np_random.uniform(
                    FOCUSED_TARGET_ANGLE_RANGE[0], FOCUSED_TARGET_ANGLE_RANGE[1]
                )
            )
        else:
...
def make_training_env() -> gym.Env:
    ...
    return ReachTrainingEnv(target_radius_range=TRAINING_TARGET_RADIUS_RANGE)
