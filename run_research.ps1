...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            'If you request measurements, each `selection` cites an observed signal or explicit uncertainty, states why measuring that model is useful, and names the next decision the result could change. Position or labels such as first, latest, final, endpoint, or horizon-matched are not sufficient reasons on their own. Each `omitted_alternative` names the strongest available model left outside the request, or is null only when every available model is requested.'
