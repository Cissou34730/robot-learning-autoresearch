...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Decide whether more evidence would help resolve lineage or choose a useful next action. State the question or uncertainty, then select measurements proportionate to it and to their cost. Reuse compatible existing evidence and choose instruments for their fit to the question, not phase convention."
