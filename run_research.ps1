...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Reason in this order: inspect what happened during training; state the scientific question; identify which measurement could change the interpretation; request that measurement, or close if current evidence is sufficient. Establish the model decision and, when further research is needed, a rational next scientific direction."
            'Use the request-level `reason` to explain why every listed measurement is needed to answer the question and how its possible outcomes could change the current interpretation, model/lineage decision, or next scientific direction. Reuse compatible existing evidence from the brief and request only evidence whose absence would leave the next direction arbitrary. Comparison and task-reference measurement are optional and must be justified by the question, not phase convention.'
            ...
            "Compare the proposal's recorded expected and contradicting observations, then update lessons, limits, open questions, and conditional next steps. Explicitly state whether the current investigation is resolved and the direction changes, remains useful with a concrete next action that can differ, or is genuinely inconclusive with the unresolved distinction and decision consequence. If repeating the same hypothesis family or requesting another replication, explain why existing evidence does not resolve the investigation and what concrete next action can differ."
            ...
            "Further training is a valid next experiment after closure; do not prepare that proposal now. Omit request_final_benchmark or set it to false when a scientifically useful next experiment remains. Setting it to true ends this campaign after the official benchmark regardless of whether its verdict is goal_reached or goal_not_reached; never use that terminal result as a gate for choosing the next hypothesis. Do not run training, measurements, Git mutations, final assessment, or research/run_experiment.py; the launcher validates and executes the accepted deliverable."
        ) -join " "
    ...
    if ($null -ne $researchState.pending_evaluation_request) {
        ...
        if (-not $evaluationPlanExists) {
            ...
            $evaluationPrompt = @(
                ...
                'State the scientific question first. Use the request-level `reason` to explain why every listed measurement is needed to answer the question and how its possible outcomes could change the current interpretation, model/lineage decision, or next scientific direction. Reuse compatible existing evidence and minimize only redundant or decision-irrelevant evidence. Comparison and task-reference measurement are optional and must be justified by the question, not phase convention.'
