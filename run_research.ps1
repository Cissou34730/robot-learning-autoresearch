...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Measured policy performance supports a claim of progress: research-evaluation episodes measure the current task, while a task-reference measurement previews the final benchmark on one reused panel and is approximate and selection-contaminated. The task-reference preview is never terminal or stopping-validation evidence. Logs, code, and training/evaluation discrepancies guide the investigation."
