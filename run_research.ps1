...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Task performance supports claims of policy progress; logs, code, and training/evaluation discrepancies may all guide investigation."
            "Investigate with the operations cataloged in research/instruments.md, inspecting implementations when that helps formulate the question. Human-owned code remains read-only."
            "Decide whether more evidence would help resolve lineage or choose a useful next action. State the question or uncertainty, then select measurements proportionate to it and to their cost. Reuse compatible existing evidence and choose instruments for their fit to the scientific question, not phase convention."
    ...
    if ($null -ne $researchState.pending_researcher_decision) {
        ...
        $decisionPrompt = @(
            ...
            "In the postmortem and Scientific strategy, assess the question actually tested against the proposal's expected_observation and contradicting_observation when present. Identify partial or unexpected signals, distinguish observations from interpretations, and scope causal claims to the evidence. When the campaign continues, carry forward what this experiment eliminated and state the next direction it leaves open."
