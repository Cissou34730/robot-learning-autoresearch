...
while ($true) {
    ...
    if ($null -ne $researchState.pending_researcher_decision) {
        ...
        $decisionPrompt = @(
            ...
            "Further training is an ordinary next experiment after this closure. If the protocol's stopping rule leads you to request terminal assessment of best_known through request_final_benchmark, use previous_result_decision.reason to explain the selection and why no scientifically useful path remains."
