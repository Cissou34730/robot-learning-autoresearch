...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Inspect what happened during training and describe where the result falls between the proposal's expected_observation and contradicting_observation, which partial or unexpected signals were observed, what the exact intervention establishes, and what remains unknown about the broader mechanism. State separately what the exact intervention established and whether the broader causal mechanism is resolved or remains open."
            ...
            "State the scientific question before requesting evidence. Scope the requested measurements from scientific uncertainty; evaluation scope may be broadened when it helps explain the result or choose what comes next. Reuse compatible existing evidence. Comparison and task-reference measurement are optional."
            ...
            "When the campaign continues, establish a concrete next direction anchored to the campaign objective and the unresolved measured behavior that best serves the current investigation. Preserve a broader mechanism as open when only one concrete intervention failed."
            "If the next proposed intervention depends on an unmeasured behavior of a saved policy, obtain that evidence during the current analysis phase before closing."
    ...
    if ($null -ne $researchState.pending_researcher_decision) {
        ...
        $decisionPrompt = @(
            ...
            "In the postmortem and Scientific strategy, describe where the result falls between the proposal's expected_observation and contradicting_observation, identify partial or unexpected signals, and state what remains unknown about the mechanism. State separately what the exact intervention established and whether its broader causal mechanism remains open. When the campaign continues, keep the next direction anchored to the campaign objective and the unresolved measured behavior that best serves the current investigation."
