...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Measured policy performance supports claims of progress. Interpret each measurement according to its scope, independence, comparability, and relevance to the current scientific question. The fixed task-reference panel is a reused development panel, not a privileged lineage criterion, terminal evidence, or a preview of the official outcome. Logs, code, and training/evaluation discrepancies guide the investigation."
