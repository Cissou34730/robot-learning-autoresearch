...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Available ways to investigate include checkpoint inventory and raw-log queries, structured-artifact analysis, implementation inspection, lightweight local analysis, researcher-owned instrumentation, research evaluation, task-reference measurement, and paired comparison. Inspect implementations to understand behavior, assumptions, coverage, outputs, and limitations, including when this helps formulate the question. Human-owned code remains read-only."
            "Decide whether more evidence would help resolve lineage or choose a useful next action. State the question or uncertainty, then select measurements proportionate to the uncertainty and their cost, including exploratory characterization when useful. Reuse compatible existing evidence. Choose instruments for their fit to the scientific question, not phase convention. Task-reference measurement assesses the protected original task independently of researcher-owned evaluation, on a separate development panel; its implementation is identified in research/instruments.md."
            ...
            "Decide whether to continue development or request terminal assessment of best_known through request_final_benchmark based on progress toward the human objective, the available evidence, and plausible opportunities for policy improvement. Account for resource constraints only when explicitly specified or reported; do not assume pressure to end the campaign quickly. Selecting best_known does not decide when to stop. Uncertainty does not prohibit stopping, and another useful experiment does not make continuation mandatory. If requesting assessment, use previous_result_decision.reason to explain the policy selection, what the available evidence supports, which uncertainty you accept, and why you are ending development now. This ends the campaign after either goal_reached or goal_not_reached; the result cannot inform a later hypothesis. Do not plan further work conditional on benchmark failure."
    ...
    if ($null -ne $researchState.pending_evaluation_request) {
        ...
        if (-not $evaluationPlanExists) {
            ...
            $evaluationPrompt = @(
                ...
                "Start from the brief and instrument contract; inspect implementations to understand behavior, assumptions, coverage, outputs, and limitations, including when this helps formulate the question. Human-owned code remains read-only. Use targeted evidence extraction when it answers the question."
                'State the scientific question first. Use the request-level `reason` to explain why the measurements are useful and proportionate to the uncertainty, and how their possible outcomes could change the current interpretation, model/lineage decision, or next scientific direction. Reuse compatible existing evidence, avoid genuinely redundant measurements, and broaden the evaluation scope when it helps explain the result or choose what comes next. Choose instruments for their fit to the scientific question, not phase convention. Task-reference measurement assesses the protected original task independently of researcher-owned evaluation, on a separate development panel; its implementation is identified in research/instruments.md.'
    ...
    if ($null -ne $researchState.pending_researcher_decision) {
        ...
        $decisionPrompt = @(
            ...
            "Decide whether to continue development or request terminal assessment of best_known through request_final_benchmark based on progress toward the human objective, the available evidence, and plausible opportunities for policy improvement. Account for resource constraints only when explicitly specified or reported; do not assume pressure to end the campaign quickly. Selecting best_known does not decide when to stop. Uncertainty does not prohibit stopping, and another useful experiment does not make continuation mandatory. If requesting assessment, use previous_result_decision.reason to explain the policy selection, what the available evidence supports, which uncertainty you accept, and why you are ending development now. A true value ends the campaign after either verdict and cannot provide feedback for another hypothesis. Do not plan further work conditional on benchmark failure."
