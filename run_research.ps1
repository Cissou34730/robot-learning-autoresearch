...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            'Use the request-level `reason` to explain why every listed measurement is needed to answer the question and how its possible outcomes could change the interpretation or lineage decision. Reuse compatible existing evidence from the brief and request only evidence whose absence would leave the next direction arbitrary. Comparison and task-reference measurement are optional and must be justified by the question, not phase convention.'
