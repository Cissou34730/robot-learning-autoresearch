...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            'If you request measurements, each `selection` states why measuring that model is useful for the scientific question, names the strongest omitted alternative on the inventory data you inspected, and says why the chosen model is more informative.'
    ...
    if ($null -ne $researchState.pending_evaluation_request) {
        ...
        if (-not $evaluationPlanExists) {
            ...
            $evaluationPrompt = @(
                ...
                'Every measurement also requires its own `selection`: state why measuring that model is useful for the current scientific question, name the strongest omitted alternative on the inventory data you inspected, and say why the chosen model is more informative.'
