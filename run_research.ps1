...
while ($true) {
    ...
    if ($researchState.schema_version -eq 4 -and $null -ne $researchState.pending_analysis) {
        ...
        $analysisPrompt = @(
            ...
            "Further training is an ordinary next experiment after closure; do not prepare that proposal now. When closing, apply the stopping rule in research/program.md: weigh ending development against the strongest concrete improvement opportunity the evidence supports, not against another round of comparable measurement. If that rule leads you to request terminal assessment of best_known through request_final_benchmark, use previous_result_decision.reason to explain the selection and why no supported opportunity justifies another experiment."
    ...
    if ($null -ne $researchState.pending_researcher_decision) {
        ...
        $decisionPrompt = @(
            ...
            "Further training is an ordinary next experiment after this closure. Apply the stopping rule in research/program.md: weigh ending development against the strongest concrete improvement opportunity the evidence supports, not against another round of comparable measurement. If that rule leads you to request terminal assessment of best_known through request_final_benchmark, use previous_result_decision.reason to explain the selection and why no supported opportunity justifies another experiment."
